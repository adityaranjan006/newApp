...
import { cartesianToPolar, interpolate, polarToCartesian } from './slider-base/conversions';
...
const RoomScoreSlider = (props: SemiCircularSliderProps) => {
  ...
  const {
    ...
    linearGradient = [
      ...
      { color: "#FF0000", offset: 0.2 },     // Red
      { color: "#FFA500", offset: 0.4 },
      { color: "#FF0500", offset: 0.6 },       // Red
      { color: "#FFA500", offset: 0.8 },     // Orange
  ...
  const angleOffset: number =5
  const startAngle: number = circleType === CircleType.Bottom ? angleOffset - 180 : angleOffset ;
  const endAngle: number = circleType === CircleType.Bottom ? -angleOffset : 240 - angleOffset;
  const interpolatedAngle = interpolate(
    value,
    [minValue, maxValue],
    circleType === CircleType.Bottom
      ? [180 + angleOffset, 360 - angleOffset]
      : [0 + angleOffset, 240 - angleOffset],
  );

  const compensatePadding = 10;
  const cx = width / 2
  const cy = circleType === CircleType.Bottom ? paddingVertical / 2 - compensatePadding : height - paddingVertical + compensatePadding
  ...
  const handlePanResponderMove = ({ nativeEvent: { locationX, locationY } }: { nativeEvent: { locationX: number; locationY: number } }) => {
    ...
    // console.log('piTo2piAngleInDegree:', piTo2piAngleInDegree, 'piToMinuspiAngleInDegree:', piToMinuspiAngleInDegree);
    ...
    if (
      ...
      piTo2piAngleInDegree >= startAngle &&
    ...
    <Animated.View>
      <Svg width={width} height={height}>
        ...
        <Path
          ...
          fill="none"
          d={`M${startCoord.x} ${startCoord.y} A ${rad} ${rad} 1 ${ endAngle - startAngle <= 180 ? 0 : 1 } 1 ${endCoord.x} ${endCoord.y}`}
          strokeLinecap="round"
        />
        <AnimatedCircle
          cx={animatedValue.x}
...
export default RoomScoreSlider;
